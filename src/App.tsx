...
export default function App() {
		...
		async function boot() {
			try {
				if (!window.camaf?.auth) {
					notify({
						type: "error",
						message: "No se cargó el puente seguro de Electron",
					});
					return;
				}
				...
				const response = await Promise.race([
					...
					new Promise<{ error: string }>((resolve) =>
						window.setTimeout(
							() =>
								resolve({
									error: "Tiempo de espera agotado al consultar la sesión",
								}),
							5000,
						),
					),
				...
				notify({
					type: "error",
					message:
						error instanceof Error ? error.message : "No se pudo iniciar CAMAF",
				});
			} finally {
				setLoading(false);
			}
	...
	if (loading) {
		return <div className="app-loading-screen">Cargando CAMAF...</div>;
	}
		...
		<AuthContext.Provider value={auth}>
			{!user ? (
				...
				<Layout route={routeState.route} navigate={navigate}>
					...
					{routeState.route === "movimientos" && (
						<Movimientos notify={notify} />
					)}
